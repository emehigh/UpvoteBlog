...
const Home = () => {
    ...
    const [postAddUpvote, setPostAddUpvote] = useState([]);
    ...
    useEffect(() => {
        socket.on('add-upvote', (newPosts) => {
            setPostAddUpvote(newPosts);
            setPostRemoveLike('');
        });
    ...
    let uiPosts = postAddUpvote.length > 0 ? postAddUpvote : postRemoveLike.length > 0 ? postRemoveLike : posts;
